...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		Boolean keyIsMemberOfSet = Util.computeLogic(key, node.getNodeID().add(BigInteger.ONE), successor.getNodeID());
	...
	private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException {
		...
		for(int i = fingerTable.size()-1; i >= 0; i--) {
			...
			Boolean closestFinger = Util.computeLogic(fingerstub.getNodeID(),node.getNodeID().add(BigInteger.ONE),key.subtract(BigInteger.ONE));
